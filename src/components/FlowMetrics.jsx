...
export default function FlowMetrics({
  ...
  // Team view — average cards only (no per-user table)
    ...
    <div className="space-y-4">
      {hasData && (
        <>
          ...
          {/* Team average cards — same MemberStageCards, clickable drill-down included */}
          {mrFlows.length > 0 && (
            <MemberStageCards
              flows={mrFlows}
              stages={MR_STAGES}
              title="MR Flow (Team Avg)"
              count={mrFlows.length}
              accentColor="#00C9FF"
            />
          )}
          {issueFlows.length > 0 && (
            <MemberStageCards
              flows={issueFlows}
              stages={ISSUE_STAGES}
              title="Issue Flow (Team Avg)"
              count={issueFlows.length}
              accentColor="#A78BFA"
              isIssue
            />
          )}
